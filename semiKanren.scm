...
(define (factorize lb ub bds s pairs)
  (cond
   ((and (pair? lb) (or (pair? ub) (not ub)))
    (let*-values (((t1 bds pairs) (factorize (car lb) (and ub (car ub)) bds s pairs))
                  ((t2 bds pairs) (if bds (factorize (cdr lb) (and ub (cdr ub)) bds s pairs)
                                      (values #f #f pairs))))
