...
(define (<= u v)
  (lambda (s/c)
    (let-values (((s lbs ubs _) (semiunify u v (subst s/c) (lbs s/c) (ubs s/c) '())))
...
(define (antiunify-top u old id)
  (cond
   ((pair? u)
    (let*-values (((t1 old) (antiunify-top (car u) old id))
                  ((t2 old) (antiunify-top (cdr u) old id)))
...
(define (factorize lb ub v lbs ubs s vs)
  (cond
   ((not lb)
    (values s lbs (cons ub ubs) vs))
   ((not ub)
    (let-values (((term new-lbs) (antiunify-top (cdr lb) '() 'idk)))
      (values (cons (cons v term) s) (append new-lbs lbs) ubs vs)))
   (else
    (let*-values (((term a-s _) (antiunify (cdr lb) (cdr ub) s 'idk)))
      (let loop ((li a-s) (lbs lbs) (ubs ubs))
        (if (null? li) (values (cons (cons v term) s) lbs ubs vs)
            ;; Should I use the data from these checks?
            (let-values (((s~ lbs~ ubs~ vs~)
                          (semiunify (caaar li) (cdaar li) s lbs ubs '())))
              (if (equal? s s~)
                  (loop (cdr li)
...
(define (adjust-upper-bound v term s lbs ubs vs)
  (let ((lb (assoc v lbs))
        (ub (assoc v ubs)))
    (if ub
        ;; TODO: The anti-substitution needs to be used
        (let-values (((term anti-s _) (antiunify (cdr ub) term s 'idk)))
          (factorize lb (cons v term) v lbs ubs s vs))
        (factorize lb (cons v term) v lbs ubs s vs))))

(define (adjust-lower-bound v term s lbs ubs vs)
  (let ((lb (assoc v lbs))
        (ub (assoc v ubs)))
    (if lb
        (let-values (((term s) (unify (cdr lb) term s)))
          (factorize (cons v term) ub v lbs ubs s vs))
        (factorize (cons v term) ub v lbs ubs s vs))))

(define (semiunify l r s lbs ubs vs)
  (let ((l (walk l s))
        (r (walk r s)))
    (cond
     ((and (var? l) (assoc l vs))
      (let-values (((_ s) (unify (cdr (assoc l vs)) r s)))
        (values s lbs ubs vs)))
     ((var? l) (adjust-upper-bound l r s lbs ubs (cons (cons l r) vs)))
     ((var? r) (adjust-lower-bound r l s lbs ubs vs))
     ((and (pair? l) (pair? r))
      (let*-values (((s lbs ubs vs) (semiunify (car l) (car r) s lbs ubs vs))
                    ((s lbs ubs vs) (if s (semiunify (cdr l) (cdr r) s lbs ubs vs)
                                        (values #f lbs ubs vs))))
        (values s lbs ubs vs)))
     ((equal? l r) (values s lbs ubs vs))
     (else (values #f lbs ubs vs)))))
